#!/usr/bin/env python3
"""
turret_server_final.py

- Relative manual control (±0.5°, ±1°, ±5°) via buttons
- Dropdown populated from JSON with labels turret1.. turretN and globe1.. globeM
- "Go" button moves turret to selected target (background thread)
- Reload targets button to refresh JSON
- Zero Motors button
- /angles endpoint to poll current motor angles
- Serves a simple HTML UI (client fetches /targets)
"""
...
positions = {}
my_turret = None
processed_targets = []   # list of dicts {label, kind, az_deg, el_deg, distance, ...}
...
def build_processed_targets():
    """
    Populate processed_targets from loaded positions.
...
def setup_motors():
    ...
    # instantiate az then el so Stepper bit mapping matches your wiring
...
def wait_for_motors(az_target, el_target, timeout=None):
    ...
    start = time.time()
    if timeout is None:
        # estimate based on steps needed
...
def manual_step(axis, delta):
    """Relative motion: use Stepper.rotate(delta). Positive delta = CCW/Up per your class."""
    if axis == "az":
...
def goto_target(label):
    """Start a background thread to move to a processed target by label."""
    ...
    def worker():
        try:
            az_goal = float(tgt["az_deg"])
            el_goal = float(tgt["el_deg"])
...
# ------------------ UI HTML (client fetches /targets to populate dropdown) ------------------
def page_html():
    # plain string (not f-string) so JS braces are safe
    return """<!doctype html>
...
function step(axis, delta){
  api('/step','POST',{axis:axis, delta:String(delta)})
    .then(r=>r.json())
    .then(j=>{ if(!j.ok) alert('Step failed'); });
}
...
function populateTargets(list){
  ...
  for(const t of list){
    ...
    opt.value = t.label; opt.text = t.label + ' ('+t.kind+')';
    sel.appendChild(opt);
  }
}
